...
SSAFunction *ast_function_to_ssa(ASTNode *func_node) {
    if (!func_node || func_node->type != NODE_FUNC_DEF) return NULL;
    
    SSAFunction *func = ssa_function_create(func_node->func_def.name);
    func->entry = ssa_function_add_block(func, "entry");
    
    SSAContext *ctx = ssa_context_create(func);
    ctx->current_block = func->entry;
    
    // Convert parameters
    func->param_count = func_node->func_def.params.count;
    func->params = malloc(func->param_count * sizeof(SSARegister *));
    for (int i = 0; i < func->param_count; i++) {
        ASTNode *param = func_node->func_def.params.items[i];
        const char *param_name = (param->type == NODE_IDENT) ? param->ident.name : NULL;
        func->params[i] = ssa_register_create(func, param_name);
        if (param_name) {
            ssa_context_set_var_version(ctx, param_name, func->params[i]->id);
        }
    }
    
    // Convert function body
    ssa_convert_statement(ctx, func_node->func_def.body);
    
    // Add exit block if not already terminated
    if (!ctx->current_block->last || 
        (ctx->current_block->last->opcode != IR_RET && 
         ctx->current_block->last->opcode != IR_BR)) {
        IRInstruction *ret = ir_create_return(NULL);
        basic_block_add_instruction(ctx->current_block, ret);
    }
    
    ssa_context_free(ctx);
    return func;
}
...
SSAModule *ast_to_ssa(ASTNode *root) {
    ...
    if (root->type == NODE_BLOCK) {
        for (int i = 0; i < root->block.stmts.count; i++) {
            ASTNode *stmt = root->block.stmts.items[i];
            if (stmt->type == NODE_FUNC_DEF) {
                SSAFunction *func = ast_function_to_ssa(stmt);
                if (func) {
                    ssa_module_add_function(module, func);
                }
            }
        }
    } else if (root->type == NODE_FUNC_DEF) {
        SSAFunction *func = ast_function_to_ssa(root);
        if (func) {
            ssa_module_add_function(module, func);
        }
    }
